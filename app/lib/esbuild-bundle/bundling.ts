...
import * as base from './bundling-base';
 ...
 */
export class Bundling implements base.BundlingOptions {
  /**
   * esbuild bundled Lambda asset code
   */
  public static bundle(options: BundlingProps): Bundling {
    // path.dirname(options.depsLockFilePath)
    return new Bundling(options)
  }
  ...
  public readonly image: base.BundlingDockerImage;
  public readonly command: string[];
  public readonly environment?: { [key: string]: string };
  public readonly workingDirectory: string;
  public readonly local?: base.ILocalBundling;
  ...
  constructor(private readonly props: BundlingProps) {
    ...
    this.image = shouldBuildImage
      ? props.dockerImage ?? base.BundlingDockerImage.fromAsset(path.join(__dirname, '../lib'), {
        buildArgs: {
          ...props.buildArgs ?? {},
      ...
      : base.BundlingDockerImage.fromRegistry('dummy'); // Do not build if we don't need to

    const bundlingCommand = this.createBundlingCommand(process.cwd(), path.join(process.cwd(), 'cdktf.out'));
