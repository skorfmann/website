...
export class NodejsFunction extends Resource {
  ...
  constructor(scope: Construct, id: string, props: NodejsFunctionProps = {}) {
    ...
    this.bundle = Bundling.bundle(bundlingOptions)

    const assetPath = path.join(process.cwd(), 'cdktf.out', 'assets', path.basename(entry, '.ts'))
    this.bundle.local?.tryBundle(assetPath, {
      image: runtime.bundlingDockerImage
    })

    const zippedAssetPath = path.join(process.cwd(), 'cdktf.out', 'assets', `${path.basename(entry, '.ts')}.zip`)
    zipDirectory(assetPath, zippedAssetPath)
    this.assetPath = zippedAssetPath
    this.assetHash = FileSystem.fingerprint(zippedAssetPath, {})
